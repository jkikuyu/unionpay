...
@RestController
public class PurchaseCancelRequest extends BaseAPIController{
	...
	@ResponseBody
	public String getCancelRequest(@RequestBody Refund refund) {
		
		Map<String, String> contentData = new HashMap<String, String>();
		putData(contentData);
		String txnType = env.getProperty("cancel.txn.type");//transaction type 31-purchase cancellation
		...
		contentData.put("txnSubType", txnSubType);                     //M transaction subtype  00 by default
		String origQryId = env.getProperty("origQryId");

		contentData.put("origQryId", origQryId);   			  //[Original transaction serial number]: queryId returned by the original purchase transaction, which can be obtained from the background notification interface or transaction state query interface for purchase transaction.
		...
		Map<String,String> rspData = AcpService.post(reqData,reqUrl,encoding);//Send request messages and receive synchronous responses (the default connection timeout is 30s and the timeout for reading the returned result is 30s); Here, after calling signData, do not make any modification to the value of any key in submitFromData before calling submitUrl. Any such modification may cause failure to signature authentication.
